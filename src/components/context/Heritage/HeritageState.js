...
const HeritageState = (props) => {
  ...
  const getLatin = async () => {
    ...
    const res = await fetch("./data.json");
    const items = await res.json();
    const Latins = items.filter(
      (item) => item.region.name === "Latin America and the Caribbean"
    );
  ...
  const getEurope = async () => {
    ...
    const res = await fetch("./data.json");
    const items = await res.json();
    const Europes = items.filter(
      (item) => item.region.name === "Europe and North America"
    );
